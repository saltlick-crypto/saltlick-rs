...
mod tests {
    ...
    async fn round_trip_test() {
        for size in &[
            ...
            let mut decrypter = SaltlickDecrypterStream::new_deferred(encrypter, |_| Some(secret_key));
    ...
    async fn incomplete_stream_test() {
        ...
        let cipher_stream = tokio_stream::once(Ok::<_, io::Error>(ciphertext.freeze()));
        let mut decrypter = SaltlickDecrypterStream::new(public_key, secret_key.clone(), cipher_stream);
